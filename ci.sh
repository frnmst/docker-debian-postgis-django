...
export MAKEFILE_URL="${1}"
export MAKEFILE_CHECKSUM="${2}"
export UWSGI_INI_URL="${3}"
export UWSGI_INI_CHECKSUM="${4}"
...
${CURL} "${MAKEFILE_URL}" --output Makefile \
    && [ "$(${SHA512SUM} Makefile | ${AWK} '{print $1}')" = "${MAKEFILE_CHECKSUM}" ]
${CURL} "${UWSGI_INI_URL}" --output uwsgi.ini \
    && [ "$(${SHA512SUM} uwsgi.ini | ${AWK} '{print $1}')" = "${UWSGI_INI_CHECKSUM}" ]
